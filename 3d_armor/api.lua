...
mod.timer = 0
mod.elements = {"head", "torso", "legs", "feet"}
mod.physics = {"jump", "speed", "gravity"}
mod.attributes = {"heal", "fire", "water"}
mod.formspec = "image[2.5,0;2,4;armor_preview]"..
  -- default.gui_bg..
  -- default.gui_bg_img..
  -- default.gui_slots..
...
mod.def = armor_def
mod.textures = armor_textures
mod.default_skin = "character"
mod.materials = {
  wood = "group:wood",
...
mod.registered_groups = {["fleshy"]=100}
mod.registered_callbacks = {
  on_update = {},
  on_equip = {},
  on_unequip = {},
  on_damage = {},
...
mod.migrate_old_inventory = true
mod.version = "0.4.13"
...
mod.register_armor = function(self, name, def)
  core.register_tool(name, def)
end

mod.register_armor_group = function(self, group, base)
  base = base or 100
...
mod.register_on_update = function(self, func)
  if type(func) == "function" then
    table.insert(self.registered_callbacks.on_update, func)
  end
end

mod.register_on_equip = function(self, func)
  if type(func) == "function" then
    table.insert(self.registered_callbacks.on_equip, func)
  end
end

mod.register_on_unequip = function(self, func)
  if type(func) == "function" then
    table.insert(self.registered_callbacks.on_unequip, func)
  end
end

mod.register_on_damage = function(self, func)
  if type(func) == "function" then
    table.insert(self.registered_callbacks.on_damage, func)
  end
end

mod.register_on_destroy = function(self, func)
  if type(func) == "function" then
    table.insert(self.registered_callbacks.on_destroy, func)
  end
end

mod.run_callbacks = function(self, callback, player, index, stack)
  if stack then
...
mod.update_player_visuals = function(self, player)
  if not player then
    return
  end
...
mod.set_player_armor = function(self, player)
  ...
  local preview = mod:get_preview(name)
  ...
  for group, level in pairs(levels) do
    if level > 0 then
      level = level * mod.config.level_multiplier
...
mod.punch = function(self, player, hitter, time_from_last_punch, tool_capabilities)
  ...
  for i, stack in pairs(list) do
    if stack:get_count() == 1 then
      ...
      local use = core.get_item_group(name, "armor_use") or 0
      local damage = use > 0
      local def = stack:get_definition() or {}
      ...
      if damage == true and hitter == "fire" then
        damage = core.get_item_group(name, "flammable") > 0
      end
      if damage == true then
...
mod.damage = function(self, player, index, stack, use)
...
mod.get_player_skin = function(self, name)
  ...
  return mod.default_skin..".png"
end

mod.add_preview = function(self, preview)
  skin_previews[preview] = true
end

mod.get_preview = function(self, name)
  local preview = string.gsub(mod:get_player_skin(name), ".png", "_preview.png")
...
mod.get_armor_formspec = function(self, name, listring)
  if mod.def[name].init_time == 0 then
    return "label[0,0;Armor not initialized!]"
  end
  local formspec = mod.formspec..
    "list[detached:"..name.."_armor;armor;0,0.5;2,3;]"
  if listring == true then
    formspec = formspec.."listring[current_player;main]"..
      "listring[detached:"..name.."_armor;armor]"
  end
  formspec = formspec:gsub("armor_preview", mod.textures[name].preview)
  formspec = formspec:gsub("armor_level", mod.def[name].level)
  for _, attr in pairs(self.attributes) do
    formspec = formspec:gsub("armor_attr_"..attr, mod.def[name][attr])
  end
  for group, _ in pairs(self.registered_groups) do
    formspec = formspec:gsub("armor_group_"..group,
      mod.def[name].groups[group])
  end
  return formspec
end

mod.get_element = function(self, item_name)
  for _, element in pairs(self.elements) do
...
mod.serialize_inventory_list = function(self, list)
  local list_table = {}
...
mod.deserialize_inventory_list = function(self, list_string)
...
mod.load_armor_inventory = function(self, player)
...
mod.save_armor_inventory = function(self, player)
...
mod.update_inventory = function(self, player)
  -- DEPRECATED: Legacy inventory support
end

mod.set_inventory_stack = function(self, player, i, stack)
...
mod.get_valid_player = function(self, player, msg)
  msg = msg or ""
  if not player then
    core.log("warning", S("3d_armor: Player reference is nil @1", msg))
    return
  end
  local name = player:get_player_name()
  if not name then
    core.log("warning", S("3d_armor: Player name is nil @1", msg))
    return
  end
  local inv = core.get_inventory({type="detached", name=name.."_armor"})
  if not inv then
    core.log("warning", S("3d_armor: Detached armor inventory is nil @1", msg))
    return
  end
  return name, inv
end

mod.drop_armor = function(pos, stack)
